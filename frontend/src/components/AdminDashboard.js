const AdminDashboard = {
  ...
  <div class="portal-page py-4 role-shell">
        ...
        <div class="row g-2">
          <div class="col-md-4">
            <select v-model="appStatus" class="form-select" @change="fetchApplications">
              <option value="">All statuses</option>
              <option v-for="s in ['applied','interview','offered','joined','offer_withdrawn','void_joined_elsewhere','rejected']"
                :key="s" :value="s">{{ formatStatusLabel(s) }}</option>
            </select>
          </div>
        </div>
      </div>
        ...
        <table class="pp-table">
          ...
          <tbody>
            <tr v-for="a in applications" :key="a.id">
              ...
              <td><span :class="'status-badge '+a.status">{{ formatStatusLabel(a.status) }}</span></td>
              <td>
      ...
      <div v-else-if="report" class="row g-4">
        ...
        <div class="col-12 col-lg-6">
          <div class="pp-card h-100 report-chart-card">
            <div class="d-flex align-items-start justify-content-between gap-3 mb-2">
              <div>
                <h6 class="fw-600 mb-1">Monthly Applications</h6>
                <div class="text-muted report-chart-subtitle">Trend for the last 6 months</div>
              </div>
              <span class="report-chart-tag">Monthly</span>
            </div>
            <div class="report-chart-shell">
              <canvas ref="monthlyApplicationsCanvas" aria-label="Applications per month bar chart"></canvas>
            </div>
          </div>
        </div>
        ...
        <div class="col-12 col-lg-6">
          <div class="pp-card h-100 report-chart-card">
            <div class="d-flex align-items-start justify-content-between gap-3 mb-2">
              <div>
                <h6 class="fw-600 mb-1">Branch Placement Rates</h6>
                <div class="text-muted report-chart-subtitle">Placed students as a share of each branch</div>
              </div>
              <span class="report-chart-tag">Branch-wise</span>
            </div>
            <div class="report-chart-shell report-chart-shell-compact">
              <canvas ref="branchPlacementCanvas" aria-label="Branch-wise placement rates pie chart"></canvas>
            </div>
          </div>
        </div>
        ...
        <div class="col-12 col-lg-6">
          <div class="pp-card h-100">
            ...
            <div v-for="entry in orderedStatusEntries(report.drives_by_status, 'drives')" :key="entry.key"
              class="d-flex justify-content-between align-items-center py-2 border-bottom">
              <span :class="'status-badge '+entry.key">{{ formatStatusLabel(entry.key) }}</span>
              <span class="fw-500">{{ entry.value }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="col-12 col-lg-6">
          <div class="pp-card h-100">
            ...
            <div v-for="entry in orderedStatusEntries(report.applications_by_status, 'applications')" :key="entry.key"
              class="d-flex justify-content-between align-items-center py-2 border-bottom">
              <span :class="'status-badge '+entry.key">{{ formatStatusLabel(entry.key) }}</span>
              <span class="fw-500">{{ entry.value }}</span>
            </div>
    ...
    <div v-if="applicationReviewModal.show"
      ...
      <div class="pp-card" style="width:100%;max-width:760px;max-height:90vh;overflow:auto;">
        <div class="d-flex justify-content-between align-items-start mb-3 gap-3">
          <div>
            <h6 class="fw-600 mb-1">Application #{{ applicationReviewModal.application?.id || '—' }}</h6>
            <div class="text-muted" style="font-size:.86rem;">{{ applicationReviewModal.application?.drive_title || '—' }}</div>
          </div>
          <span :class="'status-badge ' + (applicationReviewModal.application?.status || 'applied')">
            {{ formatStatusLabel(applicationReviewModal.application?.status) }}
          </span>
        </div>
        ...
        <div v-else class="row g-3">
          ...
          <div class="col-md-6">
            <div class="text-muted text-uppercase" style="font-size:.68rem;letter-spacing:.08em;font-weight:700;">Drive</div>
            <div class="fw-500">{{ applicationReviewModal.drive?.title || applicationReviewModal.application?.drive_title || '—' }}</div>
          </div>
          <div class="col-md-6">
            <div class="text-muted text-uppercase" style="font-size:.68rem;letter-spacing:.08em;font-weight:700;">Company</div>
            <div class="fw-500">{{ applicationReviewModal.drive?.company_name || applicationReviewModal.application?.company_name || '—' }}</div>
          </div>
          <div class="col-md-4">
            <div class="text-muted text-uppercase" style="font-size:.68rem;letter-spacing:.08em;font-weight:700;">Job Type</div>
            <div class="fw-500">{{ applicationReviewModal.drive?.job_type || '—' }}</div>
          </div>
          <div class="col-md-4">
            <div class="text-muted text-uppercase" style="font-size:.68rem;letter-spacing:.08em;font-weight:700;">Location</div>
            <div class="fw-500">{{ applicationReviewModal.drive?.location || '—' }}</div>
          </div>
          <div class="col-md-4">
            <div class="text-muted text-uppercase" style="font-size:.68rem;letter-spacing:.08em;font-weight:700;">Package</div>
            <div class="fw-500">{{ applicationReviewModal.drive?.salary_lpa ? ('₹' + applicationReviewModal.drive.salary_lpa + ' LPA') : '—' }}</div>
          </div>
          <div class="col-md-6">
            <div class="text-muted text-uppercase" style="font-size:.68rem;letter-spacing:.08em;font-weight:700;">Applied At</div>
            <div class="fw-500">{{ formatDate(applicationReviewModal.application?.applied_at) }}</div>
          </div>
          <div class="col-md-6">
            <div class="text-muted text-uppercase" style="font-size:.68rem;letter-spacing:.08em;font-weight:700;">Last Updated</div>
            <div class="fw-500">{{ formatDate(applicationReviewModal.application?.updated_at) }}</div>
          </div>

          <div class="col-12" v-if="applicationReviewModal.application?.remarks">
            <div class="text-muted text-uppercase" style="font-size:.68rem;letter-spacing:.08em;font-weight:700;">Remarks / Notes</div>
            <div class="portal-panel mt-1" style="white-space:pre-wrap;font-size:.88rem;">{{ applicationReviewModal.application.remarks }}</div>
          </div>
  ...
  data() {
    return {
      ...
      applicationReviewModal: { show: false, loading: false, application: null, student: null, drive: null },
      blacklistModal: { show: false, type: "", item: {}, reason: "" },
      rejectValidation: { submitted: false },
      blacklistValidation: { submitted: false },
      monthlyApplicationsChart: null,
      branchPlacementChart: null,
      _timers: {},
      _onWindowFocus: null,
    };
  },
  ...
  methods: {
    ...
    async openApplicationReview(application) {
      ...
      try {
        const { data } = await ApiService.adminApplication(application.id);
        const full = data.application || {};
        this.applicationReviewModal.application = full;
        this.applicationReviewModal.student = full.student || null;
        this.applicationReviewModal.drive = full.drive || null;
      } catch (e) {
    ...
    closeApplicationReview() {
      this.applicationReviewModal = { show: false, loading: false, application: null, student: null, drive: null };
    },
    ...
    async fetchReport(silent = false) {
      ...
      try {
        ...
        await this.$nextTick();
        if (this.report) this.renderReportCharts();
      }
    },

    destroyReportCharts() {
      if (this.monthlyApplicationsChart) {
        this.monthlyApplicationsChart.destroy();
        this.monthlyApplicationsChart = null;
      }
      if (this.branchPlacementChart) {
        this.branchPlacementChart.destroy();
        this.branchPlacementChart = null;
      }
    },

    renderReportCharts() {
      if (!window.Chart || !this.report) return;
      this.destroyReportCharts();
      this.renderMonthlyApplicationsChart();
      this.renderBranchPlacementChart();
    },

    renderMonthlyApplicationsChart() {
      const canvas = this.$refs.monthlyApplicationsCanvas;
      if (!canvas) return;

      const monthly = Array.isArray(this.report.monthly_applications)
        ? this.report.monthly_applications
        : [];

      const labels = monthly.map((entry) => entry.label);
      const values = monthly.map((entry) => Number(entry.count || 0));

      this.monthlyApplicationsChart = new window.Chart(canvas, {
        type: "bar",
        data: {
          labels,
          datasets: [{
    ...
    renderBranchPlacementChart() {
      const canvas = this.$refs.branchPlacementCanvas;
      if (!canvas) return;

      const branchRows = Array.isArray(this.report.branch_placement_rates)
        ? this.report.branch_placement_rates.filter((row) => Number(row.total_students || 0) > 0)
        : [];

      const labels = branchRows.map((row) => row.branch);
      const values = branchRows.map((row) => Number(row.placement_rate || 0));

      this.branchPlacementChart = new window.Chart(canvas, {
        type: "pie",
        data: {
          labels,
          datasets: [{
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              callbacks: {
                label: (ctx) => {
                  const row = branchRows[ctx.dataIndex];
                  const rate = Number(ctx.parsed || 0).toFixed(1);
                  return `${rate}% (${row.placed_students}/${row.total_students} placed)`;
                },
              },
            },
          },
    ...
    formatStatusLabel(status) {
      if (!status) return "";
      if (status === "void_joined_elsewhere") return "Joined elsewhere";
    ...
    orderedStatusEntries(statusMap, kind = "applications") {
      const source = statusMap || {};
      const keys = Object.keys(source);

      const driveOrder = ["pending", "approved", "closed", "rejected"];
      const applicationOrder = ["applied", "interview", "offered", "joined", "offer_withdrawn", "void_joined_elsewhere", "rejected"];
      const preferredOrder = kind === "drives" ? driveOrder : applicationOrder;
      ...
      return orderedKeys.map((key) => ({ key, value: source[key] }));
    },
