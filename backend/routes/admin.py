...
from models import User, Student, Company, PlacementDrive, Application, Notification, AuditLog, JobRun
...
def _status_filter_values(status):
    if not status:
        return []
    normalized = _normalized_app_status(status)
    legacy_matches = [raw for raw, mapped in LEGACY_APP_STATUS_MAP.items() if mapped == normalized]
    return [normalized, *legacy_matches]
...
def _monthly_application_counts(months=6):
    now = now_ist()
    ...
    first_month_key = month_windows[0]["key"]
    start_year, start_month = [int(value) for value in first_month_key.split("-")]
    start_dt = datetime(start_year, start_month, 1)

    rows = (
        db.session.query(
            db.func.strftime("%Y-%m", Application.applied_at).label("month_key"),
            db.func.count(Application.id).label("applications_count"),
        )
        .filter(Application.applied_at >= start_dt)
        .group_by("month_key")
        .all()
    )

    counts_by_month = {month_key: int(total or 0) for month_key, total in rows}
    for row in month_windows:
        row["count"] = counts_by_month.get(row["key"], 0)

    return month_windows


def _branch_placement_rates():
    students = db.session.query(Student.id, Student.branch).all()
    selected_students = {
        row[0]
        for row in (
    ...
    branch_counts = defaultdict(lambda: {"total_students": 0, "placed_students": 0})
    for student_id, branch in students:
        branch_name = (branch or "Unknown").strip() or "Unknown"
        branch_counts[branch_name]["total_students"] += 1
        if student_id in selected_students:
            branch_counts[branch_name]["placed_students"] += 1

    rows = []
    for branch_name, metrics in branch_counts.items():
        total_students = metrics["total_students"]
        placed_students = metrics["placed_students"]
        placement_rate = round((placed_students / total_students) * 100, 1) if total_students else 0.0
        rows.append({
            "branch": branch_name,
    ...
    rows.sort(key=lambda row: (-row["placement_rate"], -row["total_students"], row["branch"]))
    return rows
...
def list_applications():
    ...
    if status:
        query = query.filter(Application.status.in_(_status_filter_values(status)))
...
def get_application(app_id):
    ...
    if app.drive:
        data["drive"] = {
            ...
            "application_deadline": app.drive.application_deadline,
            "drive_date": app.drive.drive_date,
            "company_name": app.drive.company.name if app.drive.company else None,
        }
    else:
        data["drive"] = None
...
def reports_summary():
    ...
    return _ok({
        "report": {
            ...
            "monthly_applications":   _monthly_application_counts(months=6),
            "branch_placement_rates": _branch_placement_rates(),
...
def send_instant_report():
    ...
    try:
        ...
        from jobs.tasks import _render_monthly_report
        ...
        html = _render_monthly_report(
            ...
            report_title=f"Instant Activity Report - {now_display.strftime('%d %b %Y')}",
        )
        ...
        msg = Message(
            subject=f"CareerSync — Instant Report ({now_display.strftime('%d %b %Y %H:%M IST')})",
