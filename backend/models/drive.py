...
class PlacementDrive(db.Model):
    ...
    target_joinees      = db.Column(db.Integer)
    ...
    def joined_count(self):
        return sum(
            ...
            if app.status in ("joined", "selected", "hired")
        )

    def has_reached_target(self):
        if not self.target_joinees or self.target_joinees < 1:
            return False
        return self.joined_count() >= self.target_joinees
    ...
    def to_dict(self):
        return {
            ...
            "target_joinees":       self.target_joinees,
