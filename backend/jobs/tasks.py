...
def _normalized_app_status(status):
  return LEGACY_APP_STATUS_MAP.get(status, status)


def _aggregated_application_status_counts():
  counts = {s: 0 for s in CANONICAL_APP_STATUSES}
  rows = (
  ...
  for status, total in rows:
    normalized = _normalized_app_status(status)
    if normalized in counts:
      counts[normalized] += int(total or 0)

  filtered = {status: total for status, total in counts.items() if total > 0}
...
def send_daily_reminders():
    ...
    for student in students:
        ...
        try:
            msg = Message(
            subject="CareerSync — Application deadlines closing soon!",
...
def send_monthly_report():
    ...
    apps_by_status = _aggregated_application_status_counts()
    ...
    top_hiring_companies = [{"company": name, "selected": count} for name, count in top_companies]
    ...
    try:
        ...
        msg = Message(
          subject=f"CareerSync — Monthly Report ({now_display.strftime('%B %Y')})",
...
def export_applications_csv(student_id):
    ...
    try:
        msg = Message(
        subject="CareerSync — Your application history export",
            recipients=[student.user.email],
...
def _render_reminder_email(student, drives):
    ...
    <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:600px;margin:0 auto;background:#fff">
      <div style="background:#1a56db;padding:28px 32px">
        <h1 style="color:#fff;margin:0;font-size:20px">CareerSync</h1>
      ...
      <div style="padding:28px 32px">
        ...
        <p style="font-size:13px;color:#9ca3af">
          You are receiving this because you are registered on CareerSync.
        </p>
      </div>
    </div>"""
...
def _render_monthly_report(month, total_drives, total_applications,
                           ...
                           top_hiring_companies=None, current_date=None,
                           report_title=None):
    if current_date is None:
        current_date = now_ist()
    if top_hiring_companies is None:
    ...
    top_company_rows = ""
    for row in top_hiring_companies:
        top_company_rows += f"""
        <tr>
          <td style="padding:8px 12px;border-bottom:1px solid #e5e7eb">{row.get('company', '—')}</td>
          <td style="padding:8px 12px;border-bottom:1px solid #e5e7eb;font-weight:600">{row.get('selected', 0)}</td>
        </tr>"""
    ...
    <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:640px;margin:0 auto;background:#fff">
      <div style="background:#1a56db;padding:28px 32px">
        <h1 style="color:#fff;margin:0;font-size:20px">CareerSync</h1>
        <p style="color:#bfdbfe;margin:6px 0 0;font-size:14px">{report_title or f'Activity report — {month}'}</p>
      </div>
      <div style="padding:28px 32px">
        ...
        <table role="presentation" style="width:100%;border-collapse:separate;border-spacing:8px 0;table-layout:fixed;margin:24px 0">
          <tr>
            {"".join(f'''<td style="width:25%;vertical-align:top">
              <div style="background:#f9fafb;border-radius:10px;padding:16px;text-align:center;border:1px solid #e5e7eb">
        ...
        <table role="presentation" style="width:100%;border-collapse:separate;border-spacing:12px 0;table-layout:fixed;margin-top:24px">
          <tr>
            <td style="width:50%;vertical-align:top">
              <h3 style="font-size:14px;font-weight:600;color:#111827;margin:0 0 8px">Drives by status</h3>
              <table style="width:100%;border-collapse:collapse;font-size:13px">
                <tbody>{status_rows}</tbody>
              </table>
            </td>
            <td style="width:50%;vertical-align:top">
              <h3 style="font-size:14px;font-weight:600;color:#111827;margin:0 0 8px">Applications by status</h3>
              <table style="width:100%;border-collapse:collapse;font-size:13px">
        ...
        <div style="margin-top:24px">
          <h3 style="font-size:14px;font-weight:600;color:#111827;margin-bottom:8px">Top hiring companies</h3>
          {f'''<table style="width:100%;border-collapse:collapse;font-size:13px">
              <thead>
                <tr style="background:#f9fafb">
                  <th style="padding:8px 12px;text-align:left;color:#6b7280;font-weight:600;font-size:12px;text-transform:uppercase">Company</th>
                  <th style="padding:8px 12px;text-align:left;color:#6b7280;font-weight:600;font-size:12px;text-transform:uppercase">Students Selected</th>
                </tr>
              </thead>
              <tbody>{top_company_rows}</tbody>
            </table>''' if top_hiring_companies else '<p style="font-size:13px;color:#6b7280;margin:0">No selections yet.</p>'}
        </div>

        <p style="font-size:13px;color:#9ca3af;margin-top:28px">
          This report was auto-generated by CareerSync on {current_date.strftime("%d %b %Y at %H:%M:%S %Z")}.
        </p>
      </div>
    </div>"""
